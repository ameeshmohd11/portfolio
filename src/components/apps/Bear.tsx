    ...
    <ul>
      {items.map((item: BearMdData, index: number) => (
        <li
          ...
          className={`min-h-[5.5rem] py-3 pr-3 pl-2.5 flex flex-col justify-start cursor-default border-l-2 ${
            cur === index
              ? "border-red-500 bg-white dark:bg-gray-900"
              : "border-transparent bg-transparent"
          } hover:(bg-white dark:bg-gray-900) border-b border-c-300`}
          onClick={() => setContent(item.id, item.file, index)}
        >
          <div className="flex items-start space-x-2.5">
            <div className="w-5 flex-shrink-0 pt-0.5 text-c-500 flex justify-center">
              <span className={`${item.icon} text-base`} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between">
                <span className="font-bold text-sm text-gray-900 dark:text-gray-100 leading-snug">
                  {item.title}
                </span>
                {item.link && (
                  <a
                    className="ml-2 flex-shrink-0 text-c-500 hover:text-blue-500"
                    href={item.link}
                    target="_blank"
                    rel="noreferrer"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <span className="i-ant-design:link-outlined text-sm" />
                  </a>
                )}
              </div>
              <div className="mt-1 text-xs text-c-500 leading-relaxed">
                {item.excerpt}
              </div>
            </div>
          </div>
...
const Bear = () => {
  ...
  return (
    <div className="bear font-avenir flex h-full">
      ...
      <div className="w-64 overflow-auto" bg="gray-50 dark:gray-800" border="r c-300">
        <Middlebar
